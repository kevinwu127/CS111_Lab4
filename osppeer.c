...
#define MAXFILESIZ	52428800 // Size of largest possible file that would not 
				  // be considered a malicious attack (50 MB)
...
static task_t *task_new(tasktype_t type)
{
	...
	// Exercise 2B: use memset instead of strcopy
...
task_t *start_download(task_t *tracker_task, const char *filename)
{
	...
	// Exercise 2B: check the size of the filename to make sure it is valid
	...
	// Exercise 2A: changed strcopy to strncopy and manually added \0 to the end in order
...
static void task_download(task_t *t, task_t *tracker_task)
{
	...
	for (i = 0; i < 50; i++) {
		if (i == 0)
		{
			// Exercise 2A: Replace strcopy with strncopy to prevent buffer overflow
...
* Try 'rm %s.~*~' to remove them.\n", t->filename, t->filename);
	...
	while (1) {
		...
		// Exercise 2B: checks to make sure total file size of
		// download does not exceed defined max limit
...
static void task_upload(task_t *t)
{
	...
	// Exercise 2B: make sure the filename does not contain the '/' char
	// which would show that it would be trying to access a different directory
	// Also makes sure the filename size is within limits
	...
	size_t i;
	for(i = 0; i < strlen(t->filename); i++)
	{
		if (t->filename[i] == '/')
		{
			error("* Attempting to access invalid directory\n");
			error("* Upload terminated\n");
			goto exit;
		}	
	}
...
"         -b[MODE]     Evil mode!!!!!!!!\n");
	...
	// Exercise 1: Used fork() to create parallel processes for downloading
	...
	// Exercise 1: Used fork() to create parallel processes for uploading
